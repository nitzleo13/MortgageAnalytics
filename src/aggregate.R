...
library(sqldf)
res<-sqldf('SELECT As_of_Year, State, SUM(Loan_Amount_000) AS Total FROM Hmda GROUP BY As_of_Year, State')
View(res)
...
p<-ggplot(group,aes(x=State,y=Total))+
  geom_bar(aes(x=State,y=Total,fill=year), stat="identity",position="dodge")+
  scale_fill_discrete(name="Year", breaks=c(1, 2,3),labels=c("2012", "2013","2014"))+
  xlab("State")+ylab("Loan Amount (in $)")+ggtitle("Aggregate Loan Analysis for 2012-2014")
require(scales)
